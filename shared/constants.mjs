const BASE_URL = 'https://api.noroff.dev';

export const  LOGIN_URL =`${BASE_URL}/api/v1/social/auth/login`;
export const REGISTER_URL =`${BASE_URL}/api/v1/social/auth/register`;
export  const POSTS_URL =`${BASE_URL}/api/v1/social/posts`;
export const defaultProfileImage ="https://images.unsplash.com/photo-1516466723877-e4ec1d736c8a"
//export const SINGLE_POST_URL = `${BASE_URL}/api/v1/social/posts/<id>`;